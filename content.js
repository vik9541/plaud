...
const translations = {
  ...
  "Speaker 3": "Спикер 3",
  "Speaker 4": "Спикер 4",
  "Unknown Speaker": "Неизвестный спикер",
  "Multiple Speakers": "Несколько спикеров",
  "Waveform": "Волновая форма",
  "Timeline": "Временная шкала",
  "Timestamp": "Временная метка",
  "Add Timestamp": "Добавить временную метку",
  
  // ==================== ТРАНСКРИПЦИЯ И AI ====================
  "Transcribing": "Транскрибирование",
  "Transcribing...": "Транскрибирование...",
  "Generating summary": "Создание резюме",
  "Generating summary...": "Создание резюме...",
  "AI-powered": "На основе AI",
  ...
  "Valid": "Действительно",
  "Error in field": "Ошибка в поле",
  
  // ==================== ПОИСК И ФИЛЬТРЫ ====================
  "Search...": "Поиск...",
  "Search here": "Поиск здесь",
  "Search recordings": "Поиск записей",
  ...
  "Plaud": "Plaud",
  "Plaud AI": "Plaud AI",
  "PLAUD NOTE": "PLAUD NOTE",
  "PLAUD NotePin": "PLAUD NotePin",
  "Connect Device": "Подключить устройство",
  "Device": "Устройство",
  "Devices": "Устройства",
  ...
  "Generate Mind Map": "Создать ментальную карту",
  "Flashcards": "Карточки",
  "Generate Flashcards": "Создать карточки",
  "Template": "Шаблон",
  "Templates": "Шаблоны",
  ...
  "Recovery Email": "Email для восстановления",
  "Recovery Phone": "Телефон для восстановления",
...
// Хранилище для подсчёта переведённых элементов
...
function translatePage() {
  ...
  // Переводим alt атрибуты изображений
  const images = document.querySelectorAll('img[alt]:not([data-translated-alt])');
  images.forEach(img => {
    const originalAlt = img.getAttribute('alt');
    const translatedAlt = translateText(originalAlt);
    if (translatedAlt !== originalAlt) {
      img.setAttribute('alt', translatedAlt);
      img.setAttribute('data-translated-alt', 'true');
      newTranslations++;
    }
  });

  // Переводим кнопки
  const buttons = document.querySelectorAll('button:not([data-translated-btn])');
  buttons.forEach(button => {
    const originalText = button.textContent.trim();
    const translatedText = translateText(originalText);
    if (translatedText !== originalText && !button.querySelector('*')) {
      button.textContent = translatedText;
      button.setAttribute('data-translated-btn', 'true');
      newTranslations++;
    }
  });
  ...
  if (newTranslations > 0) {
    chrome.storage.local.set({ translatedCount: translatedCount });
  }
}
...
chrome.storage.sync.get(['translationEnabled'], function(result) {
  if (result.translationEnabled !== false) {
    ...
    console.log('✅ Plaud.ai Russian Translation v1.1.0 активировано');
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getStats') {
    sendResponse({ 
      ...
      dictionarySize: Object.keys(translations).length
    });
  }
  return true;
});
